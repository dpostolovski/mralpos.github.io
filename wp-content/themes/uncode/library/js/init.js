...
function whichTransitionEvent() {
	...
	(function(window, undefined) {
		...
		initBox = function() {
			...
			setRowHeight = function(container) {
				...
				for (var i = 0; i < container.length; i++) {
					...
					if (!!percentHeight || !!minHeight || (isIE && classie.hasClass(el, 'unequal'))) {
						child.style.height = '';
						if (!!percentHeight) {
						...
						if (classie.hasClass(el, 'row-slider')) {
							...
							if (!!minHeight) {
								if (currentTallest < minHeight) currentTallest = parseInt(minHeight);
							}
							if (isHeader || isFirst) {
